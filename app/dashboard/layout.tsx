...
const NAV_ITEMS = [
  ...
  { href: '/dashboard/advisor',      label: 'AI Advisor',   icon: '✦', highlight: true },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f1f4f6', fontFamily: 'DM Sans, sans-serif' }}>
      <aside style={{
        width: collapsed ? '64px' : '224px', flexShrink: 0, background: '#111820',
        ...
        <div style={{ padding: collapsed ? '1.25rem 0' : '1.25rem 1rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', minHeight: '64px' }}>
          {!collapsed && (
            <Link href="/dashboard" style={{ textDecoration: 'none', flex: 1 }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '19px', letterSpacing: '-0.03em' }}>
        ...
        {!collapsed && (
          <div style={{ padding: '0.875rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #14b8a6, #10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, color: '#042f2e', flexShrink: 0 }}>A</div>
            <div>
            ...
            return (
              <Link key={item.href} href={item.href} style={{ textDecoration: 'none', display: 'block' }}>
                <div title={collapsed ? item.label : undefined} style={{
                  ...
                  background: active ? 'rgba(20,184,166,0.12)' : item.highlight && !active ? 'rgba(20,184,166,0.06)' : 'transparent',
                  border: active ? '1px solid rgba(20,184,166,0.25)' : item.highlight && !active ? '1px solid rgba(20,184,166,0.12)' : '1px solid transparent',
                  color: active ? '#5eead4' : item.highlight ? '#99f6df' : '#9aaab4',
                  ...
                  {!collapsed && item.highlight && !active && (
                    <span style={{ marginLeft: 'auto', fontSize: '9px', padding: '1px 5px', background: 'rgba(20,184,166,0.2)', color: '#5eead4', borderRadius: '3px', fontWeight: 600 }}>AI</span>
                  )}
                </div>
              </Link>
            )
      ...
      <main style={{ flex: 1, marginLeft: collapsed ? '64px' : '224px', minHeight: '100vh', display: 'flex', flexDirection: 'column', transition: 'margin-left 0.25s cubic-bezier(0.16,1,0.3,1)' }}>
        <header style={{ height: '60px', background: 'white', borderBottom: '1px solid #e5eaed', padding: '0 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <Link href="/dashboard/settings" style={{ textDecoration: 'none' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #14b8a6, #10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: '#042f2e', cursor: 'pointer' }}>A</div>
            </Link>
          </div>
        </header>
